...
const STORAGE_KEY = 'arip_dashboard_tour_seen';
...
const DashboardTour = () => {
  ...
  const next = () => step < TOUR_STEPS.length - 1 ? setStep(step + 1) : close();
    ...
    <>
      ...
      <div className="fixed inset-0 z-[90]" onClick={close}>
        <svg className="w-full h-full">
          ...
          <rect width="100%" height="100%" fill="hsl(167 50% 10% / 0.5)" mask="url(#tour-mask)" />
        </svg>
      </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-1.5">
            ...
            <Button size="sm" onClick={next} className="h-7 px-3 text-xs">
              {step === TOUR_STEPS.length - 1 ? 'Finish' : 'Next'} {step < TOUR_STEPS.length - 1 && <ChevronRight className="h-3 w-3 ml-0.5" />}
            </Button>
          </div>
        </div>
      </div>
